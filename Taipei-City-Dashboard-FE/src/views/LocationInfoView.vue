  ...
  <div class="location-selector">
    <div class="category-search-row">
      <div class="form-field">
		...
		<select v-model="cityFilter" id="city" class="search-box">
			<option v-for="city in cityOptions" :key="city" :value="city">{{ city }}</option>
		</select>
      </div>
      <div class="form-field">
        <label for="category">景點分類：</label>
        <select v-model="category" id="category" class="search-box">
      ...
      <div class="form-field">
        ...
        <select v-model="selectedDropdownId" id="location" class="search-box">
          <option disabled value="__placeholder__">請選擇地點</option>
          <option
    ...
    <div class="selected-tags">
		...
		<button @click="removeTag(id)" class="remove-tag">×</button>
		</span>
    </div>

    <div class="preferences">
      <h4>額外需求：</h4>
      <label><input type="checkbox" value="accessible" v-model="preferences" /> 無障礙環境</label>
      <label><input type="checkbox" value="family" v-model="preferences" /> 親子友善</label>
      <label><input type="checkbox" value="restroom" v-model="preferences" /> 附近有廁所</label>
	  <label><input type="checkbox" value="hotel" v-model="preferences" /> 附近有旅館</label>
    </div>

    <button @click="onSearchClick" class="search-button">搜尋推薦地點</button>
	...
	<div v-if="matchedResults.length">
		...
		<tbody>
			<tr v-for="(loc, index) in matchedResults" :key="loc.id" :class="{ 'top-1': index === 0, 'top-2': index === 1, 'top-3': index === 2 }">
				<td>
				...
				<td class="bar-with-label">
					<div class="bar-container">
						<div class="bar-fill car" :style="{ width: (loc.carspace * 10) + '%' }"></div>
					</div>
					<span class="bar-label">{{ loc.carspace }}</span>
				</td>
				<td class="bar-with-label">
					<div class="bar-container">
						<div class="bar-fill safety" :style="{ width: (loc.security * 10) + '%' }"></div>
					</div>
					<span class="bar-label">{{ loc.security }}</span>
				</td>
			</tr>
		</tbody>
		</table>
	...
	<!-- 部分符合的地點 -->
	<div v-if="unmatchedResults.length" style="margin-top: 2rem;">
		<h3>未完全符合額外條件</h3>
		<table class="results">
		<thead>
			<tr>
		...
		<tbody>
			<tr v-for="loc in unmatchedResults" :key="loc.id">
				<td>{{ loc.label }}</td>
				<td>{{ loc.city }}</td>
				<td>{{ loc.accessible ? '有' : '無' }}</td>
				<td>{{ loc.family ? '有' : '無' }}</td>
	...
	<p class="indicator-note">
		※ 有無障礙環境、親子友善、附近有廁所、附近有旅館等指標，為查詢附近 1 公里內有無相關設施之情形。<br>
  		※「車位數指標」代表該地點在縣市中的停車便利程度，指標 10 代表是該縣市中最容易停車的 10% 地點，指標 1 則為最難停車的 10% 地點。<br>
  		※「治安指標」來自114年竊盜案件統計， 指標 10 代表是案件最少的 10% 地點，指標 1 則為該縣市中竊盜案件較多的 10% 地點。
	</p>
...
const categoryOptions = ['全部景點', '熱門景點', '藝文古蹟', '自然生態', '夜市']
...
const locationOptions = computed(() => {
  return rawData.value.filter(loc =>
